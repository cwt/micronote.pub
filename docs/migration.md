...
- ActivityPub: `little-boxes` → `active-boxes==0.2.2` (async-first, use `*_sync` wrappers in Flask; backend `fetch_iri` override is `async` to match the base).
...
- Keep the gzip + PIL thumbnail + EXIF-strip logic byte-identical.
...
- Local ActivityPub matrix harness (new, no real fediverse needed): `scripts/ap_matrix.py` + pytest wrapper that runs against `localhost:5005` (`DEBUG_MODE=1`, admin API key) plus a tiny in-script stub ActivityPub server (second localhost port serving a fake remote actor + objects, signed with a throwaway RSA key):
