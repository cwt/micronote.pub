...
- NOTE: NeoSQLite 1.16.1 `GridOutCursor` silently drops dotted `metadata.*` filters, so `get_file` narrows by top-level `filename` and matches url/size/kind in Python instead.
