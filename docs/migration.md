...
- NOTE: NeoSQLite 1.16.1 `GridOutCursor` silently drops dotted `metadata.*` filters, so `get_file` narrows by top-level `filename` and matches url/size/kind in Python instead. (Fixed upstream in 1.16.2 — verified against source; keep the workaround until the pin is bumped past a PyPI-visible 1.16.2.)
...
- Local ActivityPub matrix harness: `scripts/stub_remote.py` (fake remote node: actor/key/note serving, strict signature verification, delivery capture) + `scripts/ap_matrix.py` driver. Implemented cases: signed follow (auto-Accept + verified delivery), signed note (stream flag, no outbound), signed like (counter), unsigned follow (fetch-fallback path). Remaining for the full matrix: announce/update/undo/delete/media/duplicates. Requires the peer at `MICRONOTE_DEBUG=1`; see README “Simulating a remote node locally”.
